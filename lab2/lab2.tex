\section{Лабораторная работа \No2. Работа с жидкокристаллическим индикатором}
Цель работы: 
\begin{itemize}
...
\subsubsection{Регистры памяти контроллера ЖКИ}
\label{RegMemLCD}
...
ЖКИ на отладочной плате STM32L - Discovery подключен таким образом, что используются биты  \textit{S40, S41} вторых регистров \textit{LCD\_RAM} в каждой группе и биты  \textit{S0 -- S27} первых регистров \textit{LCD\_RAM}. Для уменьшения количества используемых регистров, информация из битов  \textit{S40 --S43 }будет записываться в свободные биты  \textit{S28 -- S31}, используя функцию переназначения (remapping). 

\subsubsection{Блок делителей частоты}
...
\subsection{Динамическая и статическая индикация. Управление ЖКИ}
...
\subsection{Конфигурирование микроконтроллера}
\subsubsection{Регистры периферии. Запись в регистры}
...
При работе порта в режиме альтернативной функции, выходной буфер данных порта управляется сигналами, поступающими с периферии. Заголовочный файл \textit{stm32l1xx.h} библиотеки CMSIS содержит описание всех регистров периферии, а также структуры доступа к ним:
\begin{verbatim}
...
typedef struct
{
...
Для портов \textit{GPIOA, GPIOB, GPIOC} необходимо установить 1 в 0, 1, 2 разряды регистра. Установка битов в регистр с использованием битмаски:
...
\end{figure}
Установка битов в регистр с использованием битмаски:
\begin{verbatim}
...
\subsection{Настройка контроллера ЖКИ}
...
Запись в регистр \textit{ RCC\_CSR} разрешается установкой 1 в 8 разряд регистра \textit{PWR\_CR}. Установка бита с использованием битмаски:
...
Для смены источника тактирования контроллера ЖКИ (и часов RTC тоже) необходимо сначала выполнить сброс источника тактирования установкой бита \textit{RTCRST} (установкой 1 в 23 разряд) в регистре \textit{ RCC\_CSR} (Control/status register). 
...
\end{figure}
Установка бита с использованием битмаски:
\begin{verbatim}
RCC->CSR |= RCC_CSR_RTCRST;
\end{verbatim}
Или записав в регистр значение, используя оператор \verb#|=#, т.к. значение по умолчанию регистра отлично от 0x0: 
\begin{verbatim}
RCC->CSR |= 0x800000;
...
RCC->CSR &= ~0x800000;
\end{verbatim}
В качестве источника тактового сигнала выбирается внешний НЧ генератор. Для включения генератора в регистре \textit{ RCC\_CSR} необходимо установить бит \textit{LSEON} (установить 1 в 8 разряд). Установка бита с использованием битмаски:
\begin{verbatim}
...
\end{verbatim}
Выбор внешнего НЧ генератора в качестве источника тактового сигнала осуществляется установкой в группе \textit{RTCSEL[1:0]} регистра \textit{ RCC\_CSR} значения 01. Установка битов с использованием битмаски:
\begin{verbatim}
...
\end{verbatim}
В контроллере ЖКИ необходимо установить нужный режим \textit{bias}. Для этого в регистре \textit{LCD\_CR} (LCD control register) необходимо установить значение 10 в группу \textit{BIAS[1:0]}. Перед установкой бит необходимо очистить биты от <<мусора>>. Сброс битов с использованием битмаски:
\begin{verbatim}
LCD->CR &= ~LCD_CR_BIAS;
или
LCD->CR &= ~0x60;
\end{verbatim}
Выбор режима \textit{bias=1/3} с использованием битмаски:
\begin{verbatim}
...
LCD->CR &= ~0x1C;
\end{verbatim}
Устанавливаем значение 011 в группу \textit{DUTY[1:0]} регистра \textit{LCD\_CR} для режима \textit{duty=1/4}:
\begin{verbatim}
...
\end{figure}
Активируем функцию переназначения выводов. Для этого устанавливаем 1 в 7 разряд регистра \textit{LCD\_CR}:
\begin{verbatim}
...
\end{figure}
Значения коэффициентов деления частоты тактового сигнала устанавливаем равными \textit{ck\_ps = LCDCLK/16, ck\_div = ck\_ps/17}. Для этого устанавливаем 1 в 24 и в 18 разряды.
\begin{verbatim}
LCD->FCR |= 0x1040000;
...
Для установки нужного уровня контраста необходимо установить значение 010 в группу \textit{СС[1:0]}, так же предварительно очистив биты от старых значений:
\begin{verbatim}
LCD->FCR &= ~LCD_FCR_CC;  
...
\end{figure}
В качестве источника напряжения для ЖКИ выбираем внутренний \textit{step-up converter} для формирования \textit{V\_lcd}.  Для этого в первый разряд регистра \textit{LCD\_CR} (LCD control register) устанавливается значение 0. Установка с использованием битмаски:
\begin{verbatim}
LCD->CR &= ~LCD_CR_VSEL; 
или
LCD->CR &= ~0x2;
\end{verbatim}
Разрешение работы ЖКИ контроллера происходит установкой 1 в 0 разряд регистра \textit{LCD\_CR} (LCD control register). Установка с использованием битмаски:
\begin{verbatim}
LCD->CR |= LCD_CR_LCDEN;
или
LCD->CR |= 0x1;
...
\subsection{Формирование изображения на ЖКИ}



\subsubsection{Вывод информации на ЖКИ с использованием регистров \textit{LCD\_RAM}}
\begin{figure}[H]
\begin{center}
...
\end{verbatim}
После записи информации в регистры \textit{LCD\_RAM} необходимо установить бит \textit{UDR} в регистре \textit{LCD\_SR} (LCD status register) (установить 1 во 2 разряд). Установка при помощи битмаски:
\begin{verbatim}
LCD->SR |= LCD_SR_UDR;
или
LCD->SR |= 0x4;
...
\subsubsection{Вывод информации на ЖКИ с использованием библиотеки}
В составе \textit{Standard Peripheral Library} (SPL) есть библиотека для работы с LCD дисплеями, состоящая из двух файлов \textit{stm32l1xx\_lcd.c} и \textit{stm32l1xx\_lcd.h}. Инициализация контроллера происходит аналогично инициализации портов микроконтроллера в первой лабораторной работе, при помощи структуры. 
\begin{verbatim}
