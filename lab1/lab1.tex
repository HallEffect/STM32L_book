...
Добавьте в созданные папки файлы библиотек \textit{stm32l1xx\_gpio.h, stm32l1xx\_gpio.с, stm32l1xx\_rcc.c, stm32l1xx\_rcc.h.}
...
В категории  \textit{ C/C++ Compiler}, во вкладке \textit{Preprocessor} необходимо указать компилятору пути до заголовочных и исходных файлов.
...
По умолчанию IAR генерирует исполняемый файл в формате  \textit{ELF (Executable and Linkable Format)} -- формат исполняемых и компонуемых файлов, используемый во многих UNIX-подобных операционных системах. В категории  \textit{Output Converter} отметьте пункт  \textit{Generate additional output}, а в выпадающем списке  \textit{Output format} выберете  \textit{binary}.
...
Во вкладке \textit{Vector Table} необходимо указать начало таблицы векторов прерываний. Адрес может быть 0x00000000 или 0x08000000. Адрес 0x08000000 указывает на начало внутренней Flash памяти, так же этот адрес рекомендует использовать STMicroelectronics в своем руководстве UM1451 User manual. 
...
Во вкладке \textit{Memory Regions} задаются адреса начала и окончания ROM (ПЗУ) и RAM (ОЗУ). ROM - внутренняя Flash память, начинается с адреса 0х08000000. Адрес окончания у каждого микроконтроллера разный и зависит от объема Flash памяти. Адрес окончания рассчитывается по формуле:
\begin{center}
...
\begin{center}
	% \[0x20000000+(16\cdot1024)_{10}-1_{16}=0x20003FFF\footnote{STMicroelectronics, в своем руководстве UM1451 User manual, рекомендует использовать адрес 0x20004000, т.е. 0x20003FFF+1_{16}}\]
...
Для настройки встроенного программатора-отладчика в категории \textit{ST-LINK}, в качестве интерфейса, выберете пункт \textit{SWD}.


\begin{center}
\textit{Программатор --- аппаратно-программное устройство, предназначенное для записи/считывания информации в постоянное запоминающее устройство.}
\end{center}
...
C:\Users\User\Desktop\lab\stm32l1xx_gpio.c 124
\end{verbatim}

Ошибка указывает на то, что функция \textit{assert\_param()}, которая используется в библиотеке проверки своих аргументов, объявлена неявно. Для устранения ошибки создайте файл \textit{stm32l1xx\_conf.h} содержащий:
...
Поместите файл в директорию проекта и добавьте его в проект. В исходные файлы подключаемых библиотек (\textit{stm32l1xx\_rcc.c, stm32l1xx\_gpio.c}) добавьте строку:
...
В данной лабораторной работе управление периферией (светодиодами) будет происходить через библиотеку \textit{Standard Peripherals Library (SPL)}, все необходимые для этого функции находятся в файлах \textit{stm32l1xx\_gpio.c, stm32l1xx\_gpio.h, stm32l1xx\_rcc.c, stm32l1xx\_rcc.h} в папке \verb#Лабораторные работы по микроконтроллерам#
\verb#/Materials/STM32L1xx_StdPeriph_Driver#.

Светодиод LD3 подключен к порту ввода-вывода PB7, светодиод LD4 подключен к порту ввода-вывода PB6. По умолчанию периферия STM32 не работает, пока на нее не подан тактирующий сигнал. Для подачи тактирующего сигнала используется функция, прототип которой объявлен в заголовочном файле \textit{stm32l1xx\_rcc.h}:
\begin{verbatim}
...
Где \textit{AHB} --- шина, от которой происходит тактирование. Определить нужную шину можно по названию именованной константы в файле \textit{stm32l1xx\_rcc.h}:
\begin{verbatim}
...
...
В качестве первого аргумента функции используется приведенная выше именованная константа \textit{RCC\_AHBPeriph\_GPIOB} (В случае портов PB6, PB7). В качестве второго аргумента используется константа \textit{ENABLE} или \textit{DISABLE}. На это указывает комментарий в файле \textit{stm32l1xx\_rcc.с} перед объявлением функции \textit{RCC\_AHBPeriphClockCmd()}:
\begin{verbatim}
...
Для описания режимов работы порта необходимо объявить инициализированную структуру \textit{GPIO\_InitTypeDef}, которая содержит настройки порта в качестве элементов структуры. Структура описывается в заголовочном файле периферии, которую необходимо инициализировать. В данной работе используются GPIO, следовательно, информация о структуре находиться в файле \textit{stm32l1xx\_gpio.h}:
\begin{verbatim}
typedef struct
{
...
Элементы структуры имеют тип схожий с названием элемента с добавлением суффикса \textit{\_ TypeDef}. Типы определены в качестве перечислений (enum) в файле \textit{stm32l1xx\_gpio.h}:
\begin{verbatim}
typedef enum
{ 
...
\textit{GPIO\_Mode\_OUT} -- указывает, что порт является выходом. В случае если не указать тип в структуре, то при подаче питания на отладочную плату порт будет находиться в произвольном состоянии.
\textit{GPIO\_OType\_PP} -- указывает, что выход имеет два состояния (Push-Pull).

Для инициализации полученной структуры используется функция, прототип которой объявлен в заголовочном файле \textit{stm32l1xx\_gpio.h}:
\begin{verbatim}
...
\end{verbatim}
Для подачи питания на светодиод необходимо использовать функцию, прототип которой объявлен в заголовочном файле \textit{stm32l1xx\_gpio.h}:
\begin{verbatim}
void GPIO_SetBits(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin);
\end{verbatim}
В качестве аргументов функции передаются порт и именованная константа, определенная в заголовочном файле \textit{stm32l1xx\_gpio.h}, отвечающая за необходимый пин:
\begin{verbatim}
...
